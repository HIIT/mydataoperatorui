...
function MyServiceCircleCtrl ($http) {
    ...
    function drawCircle (data) {
        ...
        var svg = d3.select(".circlegraph").append("svg")
            .attr("width", diameter)
        ...
        var h = packageHierarchy(d)

        var nodes = cluster.nodes(h),
        ...
        d3.selectAll(".node").append('image')
            .attr('xlink:href', function(d, i) {
                //console.log(d)
                return  'assets/img/' + logoArray[random(6)-1] + 'logo.svg'
            } )
            .attr('y', '11')
            .attr('x', '30')
            .attr('width', '40px')
            ...
            .attr("transform", function(d) {
                if (d.x < 180) {
                    //console.log(d.x)
                    //return d.x
                    r = "rotate(" + (-d.x+90) +  ") translate(0, -10)"
                    return r
                } else {
                    return "rotate(" + (-d.x+90) +  ") translate(-90, -10)"
                }
            })
        ...
        function mouseovered(d) {
            ...
            link
                ...
                .classed("link--fadeout", function(l) { if ( (l.target != d) && (l.source != d) ) return true; })
        ...
        function packageImports(nodes) {
            ...
            nodes.forEach(function(d) {
                if (d.connections) {
                    ReceivingData = Object.keys(d.connections.ReceivingData)
                    ReceivingData.forEach(function(i) {
                        imports.push({source: map[d.name], target: map[i]});
                    });
                    DataShared = Object.keys(d.connections.DataShared)
                    DataShared.forEach(function(i) {
